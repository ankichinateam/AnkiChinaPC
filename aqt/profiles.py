...
and no other programs are accessing your profile folders, then try again."""))
    ...
    def getLocalDataSize(self):

        sz = os.path.getsize(self.collectionPath())
        sz += os.path.getsize(os.path.join(self.profileFolder(), "collection.media.db2"))
        sz += self.getFileFolderSize(os.path.join(self.profileFolder(), "collection.media"))
        return sz

    def getFileFolderSize(self,fileOrFolderPath):
            ...
            with os.scandir(fileOrFolderPath) as dirEntryList:
                for curSubEntry in dirEntryList:
                    ...
                    if curSubEntry.is_dir():
                        curSubFolderSize = self.getFileFolderSize(curSubEntryFullPath) # 5800007
    ...
    def _defaultBase(self):
        if isWin:
            ...
            # 获取系统默认目录
            return os.path.join(get_appdata(), "AnkiChina2")
        elif isMac:
            return os.path.expanduser("~/Library/Application Support/AnkiChina2")
        else:
            dataDir = os.environ.get(
                "XDG_DATA_HOME", os.path.expanduser("~/.local/share"))
            if not os.path.exists(dataDir):
                os.makedirs(dataDir)
            return os.path.join(dataDir, "AnkiChina2")

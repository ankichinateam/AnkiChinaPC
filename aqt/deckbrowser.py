...
class WebViewPay(QWidget):
    def __init__(self, browser, parent= None):
        ...
        browser.load(QUrl(_("https://www.ankichinas.com?phone=%s")%(self.brow.mw.pm.profile['syncUser'])))
...
"""
    ...
    def _get_extra_button_info(self):
        try:
            ...
            r = requests.get('https://www.ankichinas.com/api/v1/configs/1')
            resp = r.json()
...
where id > ?""", (self.mw.col.sched.dayCutoff-86400)*1000)
    ...
    def updateLocalDataSize(self):
        try:
            ...
            r = requests.put('https://www.ankichinas.com/api/v1/users/current',headers=headers,data= data)
            # print(r.status_code)
            # print(r.text)
        except:
            # behind proxy, corrupt message, etc
    ...
    def getSpace(self):
        try:
            ...
            headers["Authorization"] = _("Bearer %s"%self.mw.pm.profile['token'])
            # auth = {'Content-Type':'application/json'},data = data
            r = requests.get('https://www.ankichinas.com/api/v1/clouds/current',headers=headers)
            resp = r.json()
            # print(r.status_code)

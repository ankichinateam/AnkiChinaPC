...
Not currently enabled; click the sync button in the main window to enable."""))
    ...
    def onSyncDeauth(self):
        ...
        try:
            ...
            headers["Authorization"] = _("Bearer %s"%self.prof['token'])
            # auth = {'Content-Type':'application/json'},data = data
            r = requests.delete('https://www.ankichinas.com/api/v1/authorizations/current',headers=headers)
            # print(r.text)
        except:
            # behind proxy, corrupt message, etc
            # print("failed")
            return
        self.prof['token'] = None

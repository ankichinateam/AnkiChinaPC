...
class AnkiQt(QMainWindow):
    ...
    def setupProfile(self):
        ...
        if self.pm.meta["firstRun"]:
            ...
            import shutil
            # 配置默认模板
            addOnsPath = os.path.join(self.pm.base, 'addons21')
            if isMac:
                dir = os.path.dirname(os.path.abspath(__file__))
                baseFolder = os.path.abspath(dir + "/../../Resources/predata/ANKIChinaPlug")
            else:
                baseFolder = os.path.join(root, 'predata/ANKIChinaPlug')

            with os.scandir(baseFolder) as dirEntryList:
                for curSubEntry in dirEntryList:
                    curSubEntryFullPath = os.path.join(baseFolder, curSubEntry.name)
                    if curSubEntry.is_dir():
                        addOnsPathTarget = os.path.join(addOnsPath, curSubEntry.name)
                        if os.path.exists(addOnsPathTarget):
                            # 如果目标路径不存在原文件夹的话就创建
                            shutil.rmtree(addOnsPathTarget)
                        shutil.copytree(curSubEntryFullPath, addOnsPathTarget)

                        # shutil.move(os.path.join(root, 'predata/295889520'),addOnsPath)
                        # print(os.path.splitext(curSubEntryFullPath)[-1])
                        # if(os.path.splitext(curSubEntryFullPath)[-1] == '.apkg'):
                        #     self.handleImport(curSubEntryFullPath)
            # shutil.move(os.path.join(root, 'predata/295889520'),addOnsPath)
            # shutil.move(os.path.join(root, 'predata/1374772155'),addOnsPath)
            # shutil.move(os.path.join(root, 'predata/1899278645'),addOnsPath)
            frun = True
...
close the profile or restart Anki."""))
    ...
    def loadProfile(self, onsuccess=None):
        ...
        # mark 可以实现系统导入
        # Todo 找到第一次启动时机
        if self.pm.profile['fristProRun']:
            # import sys
            ...
            # 'D:/workspace/anki-2.1.15/predata/ANKIModel/v1.6-Leaflyer.apkg'
            if isMac:
                dir = os.path.dirname(os.path.abspath(__file__))
                baseFolder = os.path.abspath(dir + "/../../Resources/predata/ANKIChinaModel")
            else:
                baseFolder = os.path.join(root, 'predata/ANKIChinaModel')
            with os.scandir(baseFolder) as dirEntryList:
                for curSubEntry in dirEntryList:
                    curSubEntryFullPath = os.path.join(baseFolder, curSubEntry.name)
            ...
            # self.handleImport(os.path.join(root, 'predata/ANKIModel/v1.6-Leaflyer.apkg'))
            # self.handleImport(os.path.join(root, 'predata/ANKIModel/select.apkg'))
            # self.handleImport(os.path.join(root, 'predata/ANKIModel/Leo test.apkg'))
            self.pm.profile['fristProRun'] = False
...
QTreeWidget {
  background: #eee;
    ...
    def setupAutoUpdate(self):
        ...
        # vbox = QVBoxLayout()
        # vbox.setSpacing(30)
        # hlabel = QLabel(_("""<h2>更新新版本啦！</h2>"""))
        # hlabel.setAlignment(QtCore.Qt.AlignCenter)
        # vbox.addWidget(hlabel)
        # clabel = QLabel(_("""1.更改了啥啥啥\n2.更改了啥啥啥\n3.更改了啥啥啥"""))
        # clabel.setAlignment(QtCore.Qt.AlignLeft)
